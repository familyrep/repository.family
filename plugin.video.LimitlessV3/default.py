...
#############Defined Strings#############
addon_id     = 'plugin.video.LimitlessV3'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.LimitlessV3/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.LimitlessV3/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to LimitlessV3" 
			line3 = ('[COLOR blue]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('LimitlessV3', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
##########################################
def catchup():
    loginurl   = "http://www.ipcctv.org:80/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("LimitlessV3", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search LimitlessV3:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('LimitlessV3', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('LimitlessV3', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('LimitlessV3', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('LimitlessV3', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('LimitlessV3', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.LimitlessV3/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('LimitlessV3', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.LimitlessV3')
	...
	loginurl   = "http://www.ipcctv.org:80/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://www.ipcctv.org:80/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('LimitlessV3','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
