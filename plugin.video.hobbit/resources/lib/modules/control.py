...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.hobbit.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.hobbit.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.hobbit.artwork)') else setting('appearance.alt')
    if appearance == "Hobbit": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.hobbit.artwork)')
